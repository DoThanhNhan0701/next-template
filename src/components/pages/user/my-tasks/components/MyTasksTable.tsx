...
import { TableLoadingRows } from "@/components/common/TableStateDisplay";
...
export default function MyTasksTable() {
  ...
  const [activeTableTab, setActiveTableTab] = useState<"requests" | "audits">(
    "requests",
  );
  ...
  const workflowTotal = useMemo(() => {
    ...
    if (activeStatusTab === "processing") {
      ...
      if (Array.isArray(workflowProcessingResponse)) {
        return workflowProcessingResponse.length;
      }
      const fetchedTotal =
        workflowProcessingResponse.total ??
        workflowProcessingResponse.items?.length ??
        0;
      const itemsCount = workflowProcessingResponse.items?.length ?? 0;
      return itemsCount < workflowLimit ? itemsCount : fetchedTotal;
    }
    if (activeStatusTab === "history") {
      if (!workflowHistoryResponse) return 0;
      if (Array.isArray(workflowHistoryResponse)) {
        return workflowHistoryResponse.length;
      }
      const fetchedTotal =
        workflowHistoryResponse.total ??
        workflowHistoryResponse.items?.length ??
        0;
      const itemsCount = workflowHistoryResponse.items?.length ?? 0;
      return itemsCount < workflowLimit ? itemsCount : fetchedTotal;
    }
    return 0;
  }, [
  ...
  return (
    <div className="w-full h-full flex flex-col min-h-0 gap-3">
      ...
      {/* Table Type Selector Tabs */}
      <div className="flex border-b border-border/40 pb-px gap-6 mt-2">
        <button
          onClick={() => setActiveTableTab("requests")}
          className={cn(
            "pb-3 text-xs sm:text-sm font-bold tracking-wider flex items-center gap-2 border-b-2 transition-all relative cursor-pointer",
        ...
        >
          <Check size={16} />
          {tTabs("workflow_tasks")}
          <Badge
            variant={activeTableTab === "requests" ? "default" : "secondary"}
        ...
        >
          ...
          {tTabs("my_audits")}
          <Badge
            variant={activeTableTab === "audits" ? "default" : "secondary"}
            className="ml-1 text-[10px] px-1.5 py-0 rounded-full"
          >
            {auditTotal}
          </Badge>
      ...
      {/* Main content */}
      <div className="grow flex flex-col min-h-0 gap-4 pb-2">
        {isWorkflowPending || isAuditPending ? (
          <div className="grow flex flex-col animate-pulse">
            <div className="grow flex flex-col gap-2 bg-card/25 rounded-xl border border-border/40 p-4">
              <div className="flex items-center justify-between border-b border-border/40 pb-2 mb-2">
                <div className="h-4 w-32 bg-muted/60 rounded" />
                <div className="h-4 w-20 bg-muted/60 rounded" />
              </div>
              <div className="grow border border-(--surface-border-color) rounded-lg overflow-hidden">
                <Table className="w-full" stickyHeader height="100%">
                  <TableBody>
          ...
          workflowTasks.length === 0 ? (
            <div className="grow flex flex-col items-center justify-center p-8 bg-card/25 rounded-xl border border-border/40 min-h-[300px]">
            ...
            <div className="grow flex flex-col min-h-0 gap-4 bg-card/25 rounded-xl border border-border/40 p-4">
              <div className="grow border border-(--surface-border-color) rounded-lg overflow-hidden">
                <Table className="w-full" stickyHeader height="100%">
