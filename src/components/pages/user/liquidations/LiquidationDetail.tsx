...
import {
    ChevronLeft, Package, MapPin, User, Clock, FileText,
    Link2, History, CheckCircle2, XCircle, Users, CreditCard,
...
export default function LiquidationDetail({ id }: Props) {
    ...
    const { response: detail, pending } = useGet<ILiquidationFull>({
        ...
        <div className="flex flex-col px-4 pb-6 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col gap-0.5">
                    <h1 className="text-lg font-semibold text-foreground">Liquidation Detail</h1>
                    <span className="text-xs text-muted-foreground font-medium">Asset Disposal Record</span>
                </div>
            </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex flex-col gap-0.5">
                                ...
                                <span className="text-xs text-muted-foreground">
                                    {new Date(detail.liquidation_date).toLocaleDateString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric" })}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                            <div className="flex flex-col items-center px-5 py-2.5 rounded-xl bg-primary/5 border border-primary/10 min-w-[100px]">
                                ...
                                <span className="text-xl font-bold text-primary">{detail.total_value?.toLocaleString("vi-VN")} VND</span>
                            </div>
                            <div className="flex flex-col items-center px-5 py-2.5 rounded-xl bg-muted/40 border border-border/50 min-w-[80px]">
                                ...
                                <span className="text-xl font-bold text-foreground">{detail.details?.length || 0}</span>
                            </div>
                            <Badge
                                variant="outline"
                ...
                <div className="lg:col-span-2">
                    <Card className="shadow-sm border-border/50 bg-card/60 backdrop-blur-md h-full flex flex-col">
                        <CardHeader className="flex flex-row items-center gap-2 border-b border-border/50 py-3 px-4 shrink-0">
                            ...
                            <CardTitle className="text-sm font-semibold text-primary">Disposal Items</CardTitle>
                        </CardHeader>
                            ...
                            <Table className="whitespace-nowrap">
                                <TableHeader className="bg-sidebar-accent border-b border-border/50">
                                    <TableRow>
                                        ...
                                        <TableHead className="px-4 h-10 text-xs font-semibold text-right">Unit Value</TableHead>
                                        <TableHead className="px-4 h-10 text-xs font-semibold text-right">Remaining Value</TableHead>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <Card className="shadow-sm border-border/50 bg-card/60 backdrop-blur-md">
                        <CardHeader className="flex flex-row items-center gap-2 border-b border-border/50 py-3 px-4">
                            ...
                            <CardTitle className="text-sm font-semibold text-primary">General Information</CardTitle>
                        </CardHeader>
                        <CardContent className="p-4 flex flex-col gap-4">
                            <div className="flex items-center justify-between">
                                ...
                                <Badge variant="secondary" className="capitalize">{detail.liquidation_type}</Badge>
                            </div>
                            <Separator className="bg-border/40" />
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2 text-muted-foreground">
                                    <CreditCard size={15} />
                                    <span className="text-xs font-semibold uppercase tracking-wider">Buyer</span>
                                </div>
                                <span className="text-sm font-semibold text-foreground">{detail.buyer_name || "—"}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2 text-muted-foreground">
                                    <User size={15} />
                    ...
                    <Card className="shadow-sm border-border/50 bg-card/60 backdrop-blur-md">
                        <CardHeader className="flex flex-row items-center gap-2 border-b border-border/50 py-3 px-4">
                            ...
                            <CardTitle className="text-sm font-semibold text-primary">Disposal Committee</CardTitle>
                        </CardHeader>
                        <CardContent className="p-4">
                            <div className="flex flex-wrap gap-2">
                                {detail.committee?.split(",").map((name, idx) => (
                                    <Badge key={idx} variant="outline" className="bg-muted/30 border-border/50 text-xs py-1">
                                        {name.trim()}
                                    </Badge>
                                ))}
                                {!detail.committee && <span className="text-xs text-muted-foreground italic">No committee members assigned.</span>}
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <div className="p-3 rounded-xl bg-muted/30 border border-border/50 text-center">
                        <p className="text-xs text-muted-foreground italic leading-relaxed">
                            {isPending ? "The record is currently awaiting review." : "Disposal record has been processed."}
                        </p>
                    </div>
                </div>
            </div>
                                    ...
                                    <TableRow key={hist.id} className="border-border/50 hover:bg-muted/30">
                                        ...
                                        <TableCell className="px-4 py-3">
                                            <div className="flex items-center gap-2">
                                                <div className="h-7 w-7 rounded-full bg-muted flex items-center justify-center text-xs font-bold uppercase transition-transform group-hover:scale-110">
