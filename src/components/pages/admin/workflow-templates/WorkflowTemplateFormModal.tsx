...
const DOCUMENT_TYPES = [
  ...
  { value: "rental_return", label: "Rental Return", labelVi: "Hoàn trả cho thuê" },
  { value: "liquidation", label: "Liquidation", labelVi: "Thanh lý tài sản" },
  { value: "audit", label: "Audit", labelVi: "Kiểm kê tài sản" },
];

function getAssigneeType(roleId?: number | null, userId?: number | null): AssigneeType {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 col-span-2">
      <Field className="gap-1">
        ...
        <Select value={assigneeType} onValueChange={(v) => handleTypeChange(v as AssigneeType)}>
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      <Field className="gap-1">
        <FieldLabel>
          {assigneeType === "role" ? "Select Role" : assigneeType === "user" ? "Select Staff" : "Select Assignee"}
        </FieldLabel>
        {assigneeType === "role" ? (
          <Controller
...
export default function WorkflowTemplateFormModal({ templateToEdit, isOpen, onClose, onSuccess }: Props) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { name: "", document_type: "", description: "", is_active: true, is_locked: false, steps: [] },
  });
  ...
  useEffect(() => {
    ...
    if (templateToEdit) {
      ...
      form.reset({ name: "", document_type: "", description: "", is_active: true, is_locked: false, steps: [] });
    }
  }, [isOpen, templateToEdit, form]);

  const onSubmit = async (data: FormValues) => {
    const body = { ...data, steps: data.steps.map((s, i) => ({ ...s, step_order: i + 1 })) };
    const url = isEditing ? `${endpoints.TEMPLATES}${templateToEdit.id}/` : endpoints.TEMPLATES;
    const method = isEditing ? "put" : "post";

    await mutate({ url, method, body }, {
      onSuccess: (res) => { getApiSuccessMessage(res); onSuccess(res, isEditing); onClose(); },
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] h-[90vh] flex flex-col p-0 overflow-hidden">
        <DialogHeader className="p-6 pb-4 shrink-0 border-b">
          <DialogTitle>{isEditing ? "Edit Workflow" : "Create Approval Workflow"}</DialogTitle>
          <DialogDescription>
            {isEditing ? `Editing: ${templateToEdit.name}` : "Fill in the details for the new workflow."}
          </DialogDescription>
        </DialogHeader>
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid} className="gap-1">
                    ...
                    <Input {...field} placeholder="e.g. Default Transfer Workflow" />
                      ...
                      <SelectContent>
                        {DOCUMENT_TYPES.map((t) => (
                          <SelectItem key={t.value} value={t.value}>
                            <span>{t.label}</span>
                            <span className="text-muted-foreground ml-1.5">· {t.labelVi}</span>
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Field className="gap-1">
                    ...
                    <Textarea {...field} value={field.value ?? ""} placeholder="Brief description (optional)" rows={3} />
                  </Field>
                )}
              />
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Field className="gap-1 flex justify-start items-center">
                      <label className="flex items-center gap-2 text-sm text-foreground">
                        <Checkbox id="is_locked" checked={field.value} onCheckedChange={field.onChange} />
                        Lock Workflow
                      </label>
                    </Field>
                  )}
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => append({ step_order: fields.length + 1, name: "", default_assignee_role_id: null, default_assignee_user_id: null })}
                >
                  ...
                  {fields.map((field, index) => (
                    <div key={field.id} className="flex items-start gap-2 p-3 border rounded-md bg-muted/20">
                        ...
                        <Controller
                          ...
                          render={({ field: f, fieldState }) => (
                            <Field data-invalid={fieldState.invalid} className="gap-1 col-span-2">
                              ...
                              <Input {...f} placeholder="e.g. Manager Approval" />
                              {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                            </Field>
                          )}
                        />
                      ...
                      <div className="flex flex-col gap-1 pt-1 shrink-0">
                        {index > 0 && (
                          <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" onClick={() => move(index, index - 1)} title="Move up">↑</Button>
                        )}
                        {index < fields.length - 1 && (
                          <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" onClick={() => move(index, index + 1)} title="Move down">↓</Button>
                        )}
                        <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-red-500 hover:bg-red-500/10" onClick={() => remove(index)}>
                          <Trash2Icon size={13} />
                        </Button>
                      </div>
                    </div>
